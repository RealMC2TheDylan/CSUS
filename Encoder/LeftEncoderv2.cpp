...
int main(int argc, char ** argv)
{
	...
	ros::NodeHandle n;
	ros::Publisher encoderL_pub = n.advertise<std_msgs::Float32MultiArray>("ul",1);
	std_msgs::Float32 ul;
	...
	while (n.ok())
	{
		...
		if (t = period)
		{
			...
			ul = (globalCounter_L / (float)1536) / (((double)delay) / CLOCKS_PER_SEC); //rad/s
			globalCounter_L = 0;
			
			encoderL_pub.publish(ul);
		}
	}
}